...
2. **Why does the following block of code throw an error?**
  ...
  console.log(status);
```
It throws an error because ```let``` is unable to redclare its self.
...
3. **Why does the following block of code NOT throw an error?**
  ...
  console.log(status);
  ```
It does not throw an error because ```var``` is able to re-assign its self.
...
5. **What does the following code log? Explain why?**
  ...
  ```
It logs ```Kevin Durant``` because bestPlayer is an object and it's non-primitive so it's allowed to reassign values.
6. **What is the value of `b` after this code runs? Explain why this is the case.**
  ...
  console.log(b);
  ```
The value remains 10 because primitive````````````  ``````
7. **Where does the following code throw an error? What type of error? Why?**
  ...
  console.log(bffs);
  ```
It throws an ``` Assignment to constant variable``` error because the code is attemting to reassign a constant variable.
8. **Wait, why doesn't the code below throw an error?! 🧐 What does this demonstrate?**
  ...
  console.log(bffs);
  ```
It doesn't throw an error because the code is not reassigning the variable but instead mutating it with the ```push``` method.
...
10. **What does the following code log? Why?**
  ...
  ```
It logs ```The best designer in the room is Peter``` because the function will first check it's own scope for a variable before going to the global scope.
...
Laisha is also the hardest working person in the room ```because a function will first check it's own scope for a variable before going to the global scope.
...
12. **What does the following code log? Why?**
  ...
  ```
The code logs ``` 829 Jefferson Ave. Our address is 7 Marcus Garvey``` because thee parameter it the argument and it trumps initilizations.
